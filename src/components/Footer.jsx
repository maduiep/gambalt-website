      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1.5fr", gap: 40, marginBottom: 48 }}
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
            {["in", "𝕏", "f"].map(s => (
              <div key={s} style={{ width: 34, height: 34, border: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: MUTED, cursor: "pointer" }}>{s}</div>
            ))}
          </div>
        </div>
        <div>
          ...
          {[["About Us", "about"], ["Our Projects", "projects"], ["How It Works", "how-it-works"], ["Heavy Machinery Fleet", "about"], ["Contact Us", "contact"]].map(([l, p]) => (
            <p key={l} style={{ fontSize: 13, color: SUBTLE, marginBottom: 10, cursor: "pointer" }}
        ...
        <div>
          ...
          {["Road Construction", "Commercial Structures", "Earthworks & Excavation", "Bridge Engineering", "Project Management"].map(s => (
            <p key={s} style={{ fontSize: 13, color: SUBTLE, marginBottom: 10, cursor: "pointer" }}
